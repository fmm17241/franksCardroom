...
sstruct.sci_depth=sw_dpth(sstruct.sci_water_pressure*10,nanmean(ddmm2decdeg(fstruct.m_gps_lat)));
...
dz=1; maxz=60;  zz=0:dz:maxz; zz(1)=-eps;    			% use -eps so that the bin edge includes 0
...
% then interpolate lat/lon to the time in dnmean
[~,index,~] = unique(fstruct.dn);
fdn=fstruct.dn(index); flat=fstruct.m_gps_lat(index);flon=fstruct.m_gps_lon(index);
notnans=find(~isnan(flon));
lon=interp1(fdn(notnans),flon(notnans),dnmean);
lat=interp1(fdn(notnans),flat(notnans),dnmean);
