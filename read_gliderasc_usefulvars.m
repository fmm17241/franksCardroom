...
if(fid<0); error(['File ',file,' does not exist.']); end
...
line=fgetl(fid); dstruct.mname=line(15:end);
for i=1:6
  line=fgetl(fid);
end

%line=fgetl(fid); 
line2=fgetl(fid); ndigits=str2num(fgetl(fid));
...
while 1
  line=fgetl(fid); 
  if ischar(line); data=[data; str2num(line)]; else; break; end
end
fclose(fid);
dstruct.data=data;
