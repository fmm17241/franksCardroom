...
for k = 1:length(seasons)
    %Finding standard deviations/CIs of values
    SEM = std(dayStrat{1,k}(:),'omitnan')/sqrt(length(dayStrat{1,k}));  
    ts = tinv([0.025  0.975],length(dayStrat{1,k})-1);  
    CIdayStrat(k,:) = (mean(dayStrat{:,k},'all','omitnan') + ts*SEM); 
end
...
for k = 1:length(seasons)
    %Finding standard deviations/CIs of values
    SEM = std(nightStrat{1,k}(:),'omitnan')/sqrt(length(nightStrat{1,k}));  
    ts = tinv([0.025  0.975],length(nightStrat{1,k})-1);  
    CInightStrat(k,:) = mean(nightStrat{:,k},'all','omitnan') + ts*SEM; 
end
...
nexttile()
hold on
ciplot(CInightStrat(:,1),CInightStrat(:,2),1:5,'b',0.5)
ciplot(CIdayStrat(:,1),CIdayStrat(:,2),1:5,'r',0.5)
