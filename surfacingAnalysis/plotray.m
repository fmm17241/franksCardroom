...
for isz = 1 : Nsz
   for ibeam = 1 : Nalpha
      ...
      if NumTopBnc >= 1 && NumBotBnc >= 1
         p1 =plot( r, z, 'b' );    % hits both boundaries
      elseif NumBotBnc >= 1
         p2 =plot( r, z, 'r' );	   % hits bottom only
      elseif NumTopBnc >= 1
         p3 =plot( r, z,'-.k' );	   % hits surface only
      else
         p4 =plot( r, z, 'm' );    % direct path
      end
...
% legend([p1 p2 p4],{'Both Boundaries','Bottom Only','Direct'});
